...
class Test_Data(unittest.TestCase):
    ...
    def test_check_valid_path(self):
        self.assertTrue(check_valid_path(self.path), "Should return true the path is valid")
        self.assertFalse(check_valid_path("/"), "Should return false as no path is given")
    ...
    def test_get_initial_timestamp(self):
        ...
        self.assertTrue(isinstance(initial_time, datetime.datetime), "Should be a timestamp")
    ...
    def test_timestamps_correctness(self):
        for i in range(1, len(self.timestamps)):
            ...
            self.assertTrue(isinstance(current_timestamp, datetime.datetime), "Should be a timestamp")
            self.assertEqual(current_timestamp - datetime.timedelta(microseconds=1000 * 20), previous_timestamp,
                             "Should be the same")

    def test_read_wav(self):
        self.assertTrue(issubclass(np.dtype('int32').type, np.integer), "Should be an array of integers")
